...
function OntologyDetail({ ontologies }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/ontology`, {
      ...
      .then(res => {
        if (!res.ok) throw new Error('Data model not found');
        return res.json();
      })
      .then(data => {
  ...
  if (!ontology) return <div className="ontology-detail-bg"><div className="ontology-detail-card"><h2>Data model not found</h2>{error && <div style={{color:'#ff6b6b'}}>{error}</div>}</div></div>;
  ...
  return (
    <div className="ontology-detail-layout">
        ...
        {ontology.reusedOntologies && ontology.reusedOntologies.length > 0 && (
          <div className="ontology-detail-section">
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' }}
              ...
              <h3>This data model reuses ({ontology.reusedOntologies.length})</h3>
        ...
        {ontology.requiringAssets && ontology.requiringAssets.length > 0 && (
          <div className="ontology-detail-section">
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' }}
              ...
              <h3>This data model is reused by ({ontology.requiringAssets.length})</h3>
              <span style={{ fontSize: '1.2rem', color: '#075CA8' }}>
      ...
      <aside className="ontology-detail-meta">
        ...
        >
          ⇄ 
          Click here to compare data models
        </button>
        ...
        {stats && (
          <div className="registry-stats">
            <div className="registry-stat">
              ...
              <span className="registry-stat-label">data models</span>
            </div>
            <div className="registry-stat">
              <span className="registry-stat-number">{stats.memberStates}</span>
              <span className="registry-stat-label">countries</span>
            </div>
            <div className="registry-stat">
...
function CompareView() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/ontology`, {
      ...
      .then(res => { if (!res.ok) throw new Error('Data model not found'); return res.json(); })
  ...
  return (
    <div className="compare-view">
      <div className="compare-view-header">
        ...
        <h2>Compare data models</h2>
      </div>

      <div className="compare-selector">
        {renderSlot('Data model A', 'a', slugA, ontoA, loadingA, errorA, queryA, setQueryA, suggestionsA, setSuggestionsA, searchingA, setSearchingA, debounceA)}
        <div className="compare-vs">vs</div>
        {renderSlot('Data model B', 'b', slugB, ontoB, loadingB, errorB, queryB, setQueryB, suggestionsB, setSuggestionsB, searchingB, setSearchingB, debounceB)}
      </div>

      {!slugA || !slugB ? (
        <div className="compare-placeholder">
          <p>Select two data models above to start comparing.</p>
        </div>
      ) : (loadingA || loadingB) ? (
        ...
        <div className="compare-content">
          ...
          {(() => {
            ...
            return (
              <div className="compare-section">
                <h3 className="compare-section-title">Reused data models</h3>
